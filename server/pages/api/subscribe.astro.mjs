...
const POST = async ({ request }) => {
  try {
    ...
    const LISTMONK_URL = process.env.LISTMONK_URL || "https://listmonk.funnel.leinss.xyz";
    const LISTMONK_USER = process.env.LISTMONK_API_USER;
    const LISTMONK_KEY = process.env.LISTMONK_API_KEY;
    ...
    if (response.status === 409 || errorData.message?.includes("already exists")) {
      return new Response(
        JSON.stringify({
          success: true,
          message: lang === "de" ? "Diese E-Mail-Adresse ist bereits angemeldet." : "This email address is already subscribed."
        }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }
